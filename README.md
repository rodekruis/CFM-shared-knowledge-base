...
flowchart LR
    ...
    B --> C[cfm-kb translate<br/>English -> other languages]
    C --> D[Commit translations]
    D --> E[cfm-kb publish<br/>Markdown -> HTML -> EspoCRM]
```
...
src/cfm_kb/
  ...
  data_types.py       # Typed containers for Espo records (Article, Category, ...)
...
uv run cfm-kb publish                          # publish all languages to EspoCRM
```
